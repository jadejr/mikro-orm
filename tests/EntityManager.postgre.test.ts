...
describe('EntityManagerPostgre', () => {
  ...
  test('populate queries respect the root condition (query condition propagation)', async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch('select "a0".*, "f1"."uuid_pk" as "f1__uuid_pk" from "author2" as "a0" left join "book2" as "f1" on "a0"."favourite_book_uuid_pk" = "f1"."uuid_pk" and "f1"."author_id" is not null left join "book2" as "b2" on "a0"."id" = "b2"."author_id" where "b2"."title" in (?, ?)');
